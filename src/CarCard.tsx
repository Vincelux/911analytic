...
// Matches the color language used inside the detail panel (retention bar
// gradient thresholds, TrendBadge colors) so a card and its detail view
// never disagree visually.
function retentionColor(score: number): string {
  return score >= 8 ? 'text-emerald-300' : score >= 6 ? 'text-amber-300' : 'text-red-300';
}

const trendConfig = {
  up: { icon: TrendingUp, classes: 'text-emerald-300 bg-emerald-400/10 border-emerald-400/20' },
  stable: { icon: Minus, classes: 'text-sky-300 bg-sky-400/10 border-sky-400/20' },
  down: { icon: TrendingDown, classes: 'text-red-300 bg-red-400/10 border-red-400/20' },
} as const;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {car.valueAnalysis && (
          <div className="mb-3 flex items-center gap-2 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2">
            <div className="flex shrink-0 items-baseline gap-1">
              <span className={`text-sm font-semibold ${retentionColor(car.valueAnalysis.retentionScore)}`}>
                {car.valueAnalysis.retentionScore}
              </span>
              <span className="text-[10px] font-light text-white/30">/10</span>
            </div>
            <span className="shrink-0 text-[9px] uppercase tracking-wider text-white/25">{t('retentionScore')}</span>
            {(() => {
              const { icon: TrendIcon, classes } = trendConfig[car.valueAnalysis.trend];
              return (
                <div className={`ml-auto flex min-w-0 items-center gap-1 rounded-md border px-2 py-1 ${classes}`}>
                  <TrendIcon className="h-3 w-3 shrink-0" />
                  <span className="truncate text-[10px] font-medium">{car.valueAnalysis.trendLabel}</span>
                </div>
              );
            })()}
          </div>
