import type { CarListing, VigilancePoint } from './data';
...
export function getIndicativeValue(car: CarListing): number {
  ...
  const recent = history.slice(-3).map((point) => point.price).filter((price) => price > 0);
  ...
  const ratingAdjustment = 0.96 + Math.min(0.06, Math.max(0, car.sellerRating - 4) * 0.03);
...
export function getAnalysisScore(car: CarListing): number {
  const vigilance = (car.vigilance ?? []) as VigilancePoint[];
  const penalty = vigilance.reduce((sum, item) => {
    if (item.severity === 'high') return sum + 18;
    if (item.severity === 'medium') return sum + 9;
    return sum + 3;
  }, 0);
  const sellerBonus = Math.min(10, Math.max(0, (car.sellerRating - 3.5) * 10));
...
export function getAnalysisBreakdown(car: CarListing): AnalysisBreakdown[] {
  const vigilance = (car.vigilance ?? []) as VigilancePoint[];
  const vigilanceScore = Math.max(0, 100 - vigilance.reduce((sum, item) => sum + (item.severity === 'high' ? 18 : item.severity === 'medium' ? 9 : 3), 0));
  const sellerScore = Math.min(100, Math.max(0, Math.round(car.sellerRating * 20)));
  return [
